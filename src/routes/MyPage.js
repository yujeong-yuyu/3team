...
function ordersFromCart() {
  ...
  for (const it of tagged) {
    const id = it.lastOrderId; // 주문번호는 그대로 사용
    const g =
      map.get(id) ||
      {
        orderId: id,
    ...
    const item = {
      title: it.name || "-",
      image: it.thumb || it.image || `${CDN}/placeholder.png`,
      optionLabel: it.optionLabel || "",
      qty: Number(it.qty || 1),
      unitPrice: Number(it.price || 0),
      deliveryCost: Number(it.delivery || 0),
      brand: it.brand || "",
      color: it.color || "-",
      size: it.size || "-",
      orderNo: it.orderNo || "-",
    ...
    g.purchasedAt = Math.max(g.purchasedAt || 0, Number(it.purchasedAt || 0));
    g.date = fmt(g.purchasedAt);
            ...
            <div className="a_orders">
              ...
              <div className="a_order-con">
                ...
                {pagedOrders.map((o) => {
                  ...
                  const img = first.image || "/img/placeholder.png";
